...
def huffman_code(freq):
    heap = []
    for triplet, f in freq.items():
        heap.append(HuffmanElement(f, HuffmanLeaf(triplet)))
    heapq.heapify(heap)
...
def encode_data(data, codes):
    ...
    for i in range(0, len(data) - 2, 3):
        triplet = data[i:i+3]
        encoded_data += codes[triplet] 
    padding = 8 - len(encoded_data) % 8
    padding = padding if padding != 8 else 0 
    ...
    with open(output_path, 'wb') as f:
        ...
        def serialize_tree(node):
            if isinstance(node, HuffmanLeaf):
                tree_structure.append((1, node.triplet))
            else:
...
def load_encoded_file(input_path):
    with open(input_path, 'rb') as f:
        ...
        def deserialize_tree(iterator):
            flag = next(iterator)
            if flag == 1:
                return HuffmanLeaf(bytes([next(iterator), next(iterator), next(iterator)]))
            else:
...
def decode_data(encoded_bits, root):
    ...
    while i < len(encoded_bits):
        ...
        if isinstance(current_node, HuffmanLeaf):
            decoded_bytes.extend(current_node.triplet)
            current_node = root
